...
Setup & Admin Methods
init(admin: str): Initializes the contract and sets the administrator address.
...
Pass a valid EVM address into the admin parameter of the initialization prompt.
Note: Admin address — 0x... (42 characters, starts with 0x).
Step 2: Register a Wallet
In GenLayer Studio, switch your active sender account to the wallet you want to register (e.g., Account 1: 0xAAA...).
Go to Write Methods and select register_wallet.
Enter your active wallet address (0xAAA...) into the wallet field.
Important: register_wallet can only be called from the exact account specified in the parameter.
Click Send Transaction. (If you use a different address, it will revert with UNAUTHORIZED_WALLET_REGISTRATION).
Step 3: Evaluate Contributions
Remain on the same active sender account (0xAAA...).
Select the evaluate_contributor method.
Enter your registered wallet address into the wallet_address field.
...
Click Send Transaction to trigger the AI evaluation. Wait for the ACCEPTED status.
Step 4: Check Status
Go to the Read State section.
Select get_role_status.
Enter your wallet_address.
Click Call Contract to reveal your newly assigned role!
